#!/usr/bin/env bash
# gemini-skill.sh — project a canonical agent definition to a Gemini SKILL.md.
#
# Usage:
#   gemini-skill.sh <canonical-path> <output-path>
#
# Behavior:
...
#     since Gemini SKILL.md frontmatter only honors name + description (+ gemini
#     overrides).
#   - Merges runtime_overrides.gemini into projected frontmatter.
...
overrides = (fm.get("runtime_overrides") or {}).get("gemini") or {}
projected = {k: v for k, v in fm.items() if k not in STRIP}
projected.update(overrides)

# Stable order: Gemini SKILL.md convention puts name first, description second.
...
echo "[gemini-skill] projected $canonical -> $out"
